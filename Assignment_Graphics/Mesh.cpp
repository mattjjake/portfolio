...
// Constructor: Create two buffers from arrays. 
Mesh::Mesh(Vertex* vertexArr, int vertexCount, unsigned int* indexArr, int indexCount,
	Microsoft::WRL::ComPtr<ID3D11Device> device, Microsoft::WRL::ComPtr<ID3D11DeviceContext> context)
{
	// Initialize variables
	CalculateTangents(vertexArr, vertexCount, indexArr, indexCount);
	...
	device->CreateBuffer(&vbd, &initialVertexData, vertexBuff.GetAddressOf());

	// Create the INDEX BUFFER description ------------------------------------
	D3D11_BUFFER_DESC ibd = {};
	ibd.Usage = D3D11_USAGE_IMMUTABLE;
	ibd.ByteWidth = sizeof(unsigned int) * indicies;	
	...
	device->CreateBuffer(&ibd, &initialIndexData, indexBuff.GetAddressOf());
}
...
// - NOTE: You'll need to #include <fstream>
	...
	D3D11_BUFFER_DESC ibd = {};
	ibd.Usage = D3D11_USAGE_IMMUTABLE;
	ibd.ByteWidth = sizeof(UINT) * indexCounter;
	ibd.BindFlags = D3D11_BIND_INDEX_BUFFER;
	ibd.CPUAccessFlags = 0;
	ibd.MiscFlags = 0;
	ibd.StructureByteStride = 0;
...
// Empty deconstructor (smartpointers)
Mesh::~Mesh()
{
...
// Set buffers and draw correct indicies
void Mesh::Draw()
{
	// Set buffers in the input assembler
	UINT stride = sizeof(Vertex);
	UINT offset = 0;
	context->IASetVertexBuffers(0, 1, vertexBuff.GetAddressOf(), &stride, &offset);
	context->IASetIndexBuffer(indexBuff.Get(), DXGI_FORMAT_R32_UINT, 0);

	// Send number of indicies
	context->DrawIndexed(
		indicies,     
		0,    
		0);    
